...
client.on("ready", async () => {
  ...
  if (guild) {
    ...
    console.log(gradient(["red", "white"])(t('nosvr')));
    process.exit(1);
  }
  menutext(client);
  choiceinit(client);
  ...
  const r = new Discord.RichPresence()
    ...
    .setType('PLAYING')
    .setURL('https://discord.gg/bqVJZSJWgD')
    .setName('KUA')
    .setState('Running...')
    ...
    .addButton('Join', 'https://discord.gg/bqVJZSJWgD');
...
if (!token) {
  console.clear();
  creatorname();
  rl.question(gradient(["red", "pink"])("Token You (Token Bot = Eror))\n» "), (input) => {
    if (input.trim() === '') {
      console.log(gradient(["pink", "red"])("O token foi retornado como vazio"));
      process.kill(1);
    } else {
      client.login(input)
        .catch((error) => {
          if (error.message === 'An invalid token was provided.') {
            console.clear();
            console.log(gradient(["red", "pink"])("Invalid token"));
          } else {
            console.clear();
            console.error(gradient(["red", "pink"])(`Erro ao fazer login: ${error.message}`));
          }
        });
    }
  });
